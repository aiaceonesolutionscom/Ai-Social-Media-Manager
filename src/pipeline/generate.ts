...
const SYSTEM_INTENT = `You are an AI assistant that analyzes Instagram content ideas.
...
const SYSTEM_PLAN = `You are a content strategist for Instagram. Given a topic, audience, tone, goal, and emotion, propose:
...
const SYSTEM_WRITER = `You are a creative Instagram copywriter. Write a complete Instagram post given the content brief.
Rules:
...
const SYSTEM_BRAND_CHECK = `You are a brand and policy checker for Instagram posts. Review the following caption and check:
...
const SYSTEM_EDIT = `You are an Instagram post editor. Given the current post and the user's edit request, decide what needs to change and produce the changed pieces.
Scope values:
