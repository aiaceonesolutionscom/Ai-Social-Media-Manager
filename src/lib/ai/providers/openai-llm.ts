...
export const openaiLLM: LLMProviderAdapter = {
  name: 'openai',
  category: 'llm',
  ...
  async chat(apiKey: string, baseUrl: string, model: string, messages: ChatMessage[], options?: ChatOptions): Promise<ChatResult> {
    ...
    if (!res.ok) throw new Error(`OpenAI LLM failed ${res.status}: ${JSON.stringify(data.error ?? data)}`)
    const choice = data.choices?.[0]
    const content = choice?.message?.content
    if (!content) throw new Error('OpenAI LLM returned empty response')
    return {
      content,
  ...
  async testConnection(apiKey: string, baseUrl: string, model: string): Promise<TestResult> {
    ...
    try {
      ...
      return { ok: true, message: 'OpenAI LLM connection successful', latencyMs: Date.now() - start }
    } catch (err) {
      return { ok: false, message: `OpenAI LLM connection failed: ${(err as Error).message}`, latencyMs: Date.now() - start }
    }
  },
}
