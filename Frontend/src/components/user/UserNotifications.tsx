...
export function UserNotifications() {
  ...
  const fetchNotifications = React.useCallback(async () => {
    try {
      const data = await apiRequest<{ notifications: UserNotification[]; unreadCount: number }>(endpoints.notifications);
      setNotifications(data.notifications || []);
      setUnreadCount(data.unreadCount || 0);
    } catch {
      // Degrade gracefully (e.g. no token / backend down).
    }
  }, []);
  ...
  const markRead = async (id: string) => {
    ...
    try {
      await apiRequest(endpoints.notificationRead(id), { method: 'POST' });
    } catch {
      // Ignore
    }
  };
  ...
  const markAllRead = async () => {
    ...
    setUnreadCount(0);
    try {
      await apiRequest(endpoints.notificationsReadAll, { method: 'POST' });
    } catch {
      // Ignore
    }
  };
  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        onClick={toggleOpen}
        className="relative flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900"
      >
